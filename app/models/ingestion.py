...
This module defines the database models for tracking data ingestion processes
and individual file processing within those jobs.
"""

from sqlalchemy import Column, String, Integer, DateTime, Text, JSON, ForeignKey, CheckConstraint, Index, UUID, BigInteger, select, func
...
class IngestJob(Base):
    ...
    # Can contain API endpoints, database connection details, file paths, cloud storage info, etc.
    ...
    # Error handling
    error_messages = Column(JSON, nullable=True)  # Array of error details with context
    ...
    files = relationship("IngestFile", back_populates="job", cascade="all, delete-orphan")
...
class IngestFile(Base):
    """
    Model for tracking individual files within an ingestion job.
    ...
    source_path = Column(String(1000), nullable=True)  # Original path in source system
    ...
    checksum = Column(String(128), nullable=True, index=True)  # File checksum/hash
    ...
    # Additional metadata
    file_metadata = Column(JSON, nullable=True)  # Additional file-specific metadata

    # Relationship to job
    job = relationship("IngestJob", back_populates="files")
