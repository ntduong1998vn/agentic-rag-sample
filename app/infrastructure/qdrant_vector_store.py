...
class QdrantVectorStore(VectorStorePort):
    ...
    async def store_vectors(
        ...
        try:
            ...
            points = [
                models.PointStruct(
                    id=vec["id"], vector=vec["vector"], payload=vec.get("payload", {})
                )
                for vec in vectors
            ]
            ...
            logger.info(
                f"Stored {len(vectors)} vectors in collection '{collection_name}'"
            )
    ...
    async def delete_vectors(
        ...
        try:
            ...
            logger.info(
                f"Deleted vectors from collection '{collection_name}' with filter: {filter_criteria}"
            )
    ...
    async def search_vectors(
        ...
        try:
            ...
            logger.info(
                f"Found {len(results)} similar vectors in collection '{collection_name}'"
            )
            return results
    ...
    async def create_collection(
        self, collection_name: str, vector_dimension: int, **kwargs
    ) -> None:
        """
        ...
        try:
            ...
            if not exists:
                ...
                logger.info(
                    f"Created Qdrant collection '{collection_name}' with dimension {vector_dimension}"
                )
            else:
